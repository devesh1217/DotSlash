...
// Function to store document on blockchain
async function storeDocument(file) {
  ...
  try {
    ...
    const data = await response.json();
    const docHash = web3.utils.soliditySha3(data["Hash"]);

    await window.contract.methods
      .addDocHash(docHash, data["Hash"])
      .send({ from: window.userAddress });
...
async function handleDocumentUpload(event) {
  ...
  if (file) {
    const ipfsHash = await storeDocument(file);
    console.log(`Document stored with IPFS hash: ${ipfsHash}`);
  }
}

// Event listener for file input change
document.getElementById("doc-file").addEventListener("change", handleDocumentUpload);
...
async function onboardUser() {
  ...
  $("#loader").removeClass("d-none");
  $("#onboard_button").slideUp();
  $("#note").html(`<h5 class="text-info">Uploading documents to IPFS...</h5>`);
  ...
  for (const file of files) {
    ...
    const data = await response.json();
    ipfsHashes.push(data["Hash"]);
  }

  $("#note").html(`<h5 class="text-info">Storing document hashes on blockchain...</h5>`);

  for (const hash of ipfsHashes) {
    const docHash = web3.utils.soliditySha3(hash);
    await window.contract.methods
      .addDocHash(docHash, hash)
      .send({ from: window.userAddress })
      .on("transactionHash", function (_hash) {
        $("#note").html(`<h5 class="text-info">Please wait for transaction to be mined...</h5>`);
      })
      .on("receipt", function (receipt) {
        console.log("Document hash stored:", receipt);
      })
      .on("error", function (error) {
        console.error("Error storing document hash:", error.message);
        $("#note").html(`<h5 class="text-center">${error.message}</h5>`);
      });
  }

  $("#loader").addClass("d-none");
  $("#onboard_button").slideDown();
  $("#note").html(`<h5 class="text-info">User onboarding completed successfully!</h5>`);
}
